     ...
     stage('application.yml copy') {
         ...
         steps {
            ...
            dir ('.') {
               ...
               then
                  ...
                  cp ../application-shop.yml ./src/main/resources/application.yml
               else
      ...
      stage('Docker Test Run') {
         ...
         post {
            always {
               sh """
               container=\$(docker ps -q --filter name=${env.DOCKER_IMAGE_NAME}_test)
               if [ -n "\$container" ]; then
      ...
      stage('Mattermost') {
         ...
         steps {
            mattermostSend(
              channel: "S001-Test",
              message: "shopping mall의 ${env.BUILD_NUMBER}번째 빌드가 성공했습니다."
           )
         }
         post {
            success {
